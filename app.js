...
// ===== State =====
let hikes = [];// ===== Local storage keys =====
const LS_HIKES = 'pyrenees_hikes';                // still used as a cache
const LS_ACCOM = 'pyrenees_accommodations';
const LS_ATTR  = 'pyrenees_attractions';
const LS_HERO  = 'pyrenees_hero_url';
const LS_BACKEND = 'pyrenees_backend_url';        // Apps Script Web App URL
...
function getBackendUrl() {
  // Your live Apps Script URL (fallback if nothing in localStorage)
  return localStorage.getItem(LS_BACKEND) || 'https://script.google.com/macros/s/AKfycbwQvaXKkgu_uusyZslagfG88WOmUy0zs5_JHsfELEvp3HUVBcI-Emv9IlhCpTb9UUvZKw/exec';
}
...
// Excel import → write to cloud (WIPE then add items from Excel ONLY)
...
document.getElementById('importXlsxInput').addEventListener('change', async (e) => {
  ...
  try {
    await cloudWipe();               // clear cloud hikes
    for (const h of json) {          // add Excel rows
      await cloudAddHike(h);
    }
...
// ===== Init =====
(async function init() {
  // Prefill inputs if stored; otherwise show your default URL in the input
  const be = localStorage.getItem(LS_BACKEND) || getBackendUrl();
  const beInput = document.getElementById('backendUrlInput');
  if (beInput) beInput.value = be;
  ...
  // Always load hikes from cloud (we have a default backend URL now)
  await refreshFromCloud();

  // Optional: auto-refresh hikes every 30s
